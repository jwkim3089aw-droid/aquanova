...
// [CHANGED] log(): #diag 없으면 console.log로도 남김
...
> {
  ...
  componentDidCatch(error: any, info: any) {
    log("[HardErrorBoundary] " + (error?.stack || error?.message || String(error)));
    if (info?.componentStack) log("[componentStack] " + info.componentStack);
  }
  render() {
    if (this.state.error) {
      return (
        <div style={{ padding: 16, background: "#fee2e2", border: "1px solid #fecaca", color: "#7f1d1d", borderRadius: 8 }}>

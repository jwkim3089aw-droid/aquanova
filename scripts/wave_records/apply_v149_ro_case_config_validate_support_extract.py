...
TARGET_FUNCTION = "_validate_case_automation_support"
TARGET_FILE_REL = Path("ro") / "case_config.py"

IMPORT_MARKER_START = "# V149_RO_CASE_CONFIG_IMPORT_START"
IMPORT_MARKER_END = "# V149_RO_CASE_CONFIG_IMPORT_END"
EXTRACT_MARKER = "# V149_RO_CASE_CONFIG_VALIDATE_SUPPORT_APPLIED"
MANIFEST_NAME = "v149_ro_case_config_validate_support_manifest.json"
...
def main() -> int:
    ...
    if needs_wave_error_factory and not wave_automation_error_is_raise_factory_safe(target_node):
        raise SystemExit("V149 target uses WaveAutomationError in a non-raise-call context; lazy factory would be unsafe.")
    ...
    if unresolved:
        raise SystemExit(
            f"V149 target is not safe to move. function={TARGET_FUNCTION} unresolved={unresolved}"
        )

    backup(legacy, "v148_before_v149")
    backup(case_config, "v148_before_v149")
    ...
    if needs_wave_error_factory and EXCEPTION_FACTORY_NAME not in existing_defs:
        bridge_blocks.append(make_wave_error_factory())
    ...
    data = {
        "schema_version": "aquanova.refactor.v149.ro_case_config_validate_support_extract",
        ...
        "note": "Moves small validation helper into ro.case_config and bridges ro.feedwater dependency.",
    }
    manifest.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")

    print("V149 RO case_config validation-support extraction applied")
    print("moved_count=1")

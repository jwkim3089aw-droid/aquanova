...
BRIDGEABLE_LEGACY_REFS = {"_get_window_rect"}
...
def main() -> int:
    ...
    if EXTRACT_MARKER in stages_text and manifest.exists():
        print("V153/V153B already applied")
    ...
    bridged_legacy_refs = sorted(
        ...
        if name in BRIDGEABLE_LEGACY_REFS and name in funcs and name not in stages_defs
    )
    ...
    if unresolved:
        raise SystemExit(
            f"V153B target is not safe to move. function={TARGET_FUNCTION} unresolved={unresolved}"
        )

    backup(legacy, "v152_before_v153b")
    backup(stages, "v152_before_v153b")
    ...
    # 3) Fallback: rewrite function to call lazy getters resolving legacy constants at call time.
    lazy_getter_refs = []
    if unresolved_refs:
    ...
    data = {
        "schema_version": "aquanova.refactor.v153b.ro_stages_map_reference_point_extract",
        ...
        "legacy": str(legacy),
        "stages": str(stages),
        "note": "Moves _map_reference_point into ro.stages. V153B supports legacy _get_window_rect plus reference constants by assignment/import/lazy getter.",
    }
    manifest.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")

    print("V153B RO stages map-reference-point extraction applied")
    print("moved_count=1")

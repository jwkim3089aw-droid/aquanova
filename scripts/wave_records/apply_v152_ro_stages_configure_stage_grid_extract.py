...
TARGET_FUNCTION = "_configure_stage_grid"
TARGET_FILE_REL = Path("ro") / "stages.py"

# Preserve the active ro.stages import marker when one already exists.
FALLBACK_IMPORT_MARKER_START = "# V152_RO_STAGES_IMPORT_START"
FALLBACK_IMPORT_MARKER_END = "# V152_RO_STAGES_IMPORT_END"
EXTRACT_MARKER = "# V152_RO_STAGES_CONFIGURE_STAGE_GRID_APPLIED"
MANIFEST_NAME = "v152_ro_stages_configure_stage_grid_manifest.json"
...
def main() -> int:
    ...
    if needs_wave_error_factory and not wave_automation_error_is_raise_factory_safe(target_node):
        raise SystemExit("V152 target uses WaveAutomationError in a non-raise-call context; lazy factory would be unsafe.")
    ...
    explicit_import_deps = sorted(
        name for name in target_free
        if name in imports and name not in stages_defs and name not in bridged_ro_refs and name != EXCEPTION_FACTORY_NAME
    )
    ...
    if unresolved:
        raise SystemExit(
            f"V152 target is not safe to move. function={TARGET_FUNCTION} unresolved={unresolved}"
        )

    backup(legacy, "v151_before_v152")
    backup(stages, "v151_before_v152")
    ...
    for ref in bridged_ro_refs:
        if ref not in existing_defs:
            module = ro_index[ref]
            if module == "stages":
                raise SystemExit(f"V152 refused to bridge {ref} to ro.stages because it should already be local.")
    ...
    data = {
        "schema_version": "aquanova.refactor.v152.ro_stages_configure_stage_grid_extract",
        ...
        "legacy": str(legacy),
        "stages": str(stages),
        "note": "Moves _configure_stage_grid into ro.stages after smaller stage helpers were extracted.",
    }
    manifest.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")

    print("V152 RO stages configure-stage-grid extraction applied")
    print("moved_count=1")

...
TARGET_FUNCTION = "open_and_configure_ro_flow_case"
TARGET_FILE_REL = Path("ro") / "case_config.py"

FALLBACK_IMPORT_MARKER_START = "# V159_RO_CASE_CONFIG_IMPORT_START"
FALLBACK_IMPORT_MARKER_END = "# V159_RO_CASE_CONFIG_IMPORT_END"
EXTRACT_MARKER = "# V159_RO_CASE_CONFIG_OPEN_FLOW_CASE_APPLIED"
MANIFEST_NAME = "v159_ro_case_config_open_flow_case_manifest.json"
...
def import_marker_pairs() -> list[tuple[str, str]]:
    return [
        ("# V159_RO_CASE_CONFIG_IMPORT_START", "# V159_RO_CASE_CONFIG_IMPORT_END"),
...
def build_ro_index(ro_dir: Path) -> dict[str, str]:
    ...
    for p in sorted(ro_dir.glob("*.py")):
        ...
        for name in module_defined_names(tree):
            # Do not index plain stdlib imports as RO functions. They should be direct imports.
...
def main() -> int:
    ...
    if "WaveAutomationError" in except_handler_names(target_node):
        raise SystemExit("V159 refused: WaveAutomationError is used as an except-handler type in target.")

    safe_direct_import_deps = sorted(
        name for name in target_free
        if name in SAFE_DIRECT_IMPORTS and name not in case_defs
    )
    bridged_ro_refs = sorted(
        name for name in target_free
        if name in ro_index
        and name not in case_defs
    ...
    if unresolved:
        raise SystemExit(
            f"V159 target is not safe to move. function={TARGET_FUNCTION} unresolved={unresolved}"
        )

    backup(legacy, "v158_before_v159")
    backup(case_config, "v158_before_v159")
    ...
    for dep in safe_direct_import_deps:
        ...
        if block not in case_text and block not in blocks:
            blocks.append(block)
    ...
    data = {
        "schema_version": "aquanova.refactor.v159.ro_case_config_open_flow_case_extract",
        ...
        "note": "Moves open_and_configure_ro_flow_case into ro.case_config and treats logging as an explicit safe import.",
    }
    manifest.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")

    print("V159 RO case_config open-flow-case extraction applied")
    print("moved_count=1")

...
TARGET_FUNCTION = "_stage_cell_point"
TARGET_FILE_REL = Path("ro") / "stages.py"

# Keep the existing ro.stages import-block marker when one already exists.
# This avoids breaking older selftests every time a new stages helper is moved.
FALLBACK_IMPORT_MARKER_START = "# V150_RO_STAGES_IMPORT_START"
FALLBACK_IMPORT_MARKER_END = "# V150_RO_STAGES_IMPORT_END"
EXTRACT_MARKER = "# V150_RO_STAGES_STAGE_CELL_POINT_APPLIED"
MANIFEST_NAME = "v150_ro_stages_stage_cell_point_manifest.json"
...
def main() -> int:
    ...
    if needs_wave_error_factory and not wave_automation_error_is_raise_factory_safe(target_node):
        raise SystemExit("V150 target uses WaveAutomationError in a non-raise-call context; lazy factory would be unsafe.")
    ...
    # Existing _stage_cell_point bridge in ro/stages will be replaced by the real function.
    unresolved.discard(TARGET_FUNCTION)
    unresolved = sorted(x for x in unresolved if x and not keyword.iskeyword(x))
    if unresolved:
        raise SystemExit(
            f"V150 target is not safe to move. function={TARGET_FUNCTION} unresolved={unresolved}"
        )

    backup(legacy, "v149_before_v150")
    backup(stages, "v149_before_v150")
    ...
    # Replace earlier bridge def _stage_cell_point, if present, with the real function.
    ...
    active_start, active_end = active_import_markers(read_text(legacy))
    data = {
        "schema_version": "aquanova.refactor.v150.ro_stages_stage_cell_point_extract",
        ...
        "legacy": str(legacy),
        "stages": str(stages),
        "note": "Moves _stage_cell_point into ro.stages and replaces the previous bridge created by V140.",
    }
    manifest.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")

    print("V150 RO stages stage-cell-point extraction applied")
    print("moved_count=1")

...
IMPORT_MARKER_START = "# V145_RO_STAGES_IMPORT_START"
IMPORT_MARKER_END = "# V145_RO_STAGES_IMPORT_END"
EXTRACT_MARKER = "# V145_RO_STAGES_STABILIZE_AFTER_FLOW_COMMIT_APPLIED"
MANIFEST_NAME = "v145_ro_stages_stabilize_after_flow_commit_manifest.json"

# These are larger legacy helpers with logging/_fmt_value/UI dependencies.
# Bridge them for now instead of moving them together.
...
def main() -> int:
    ...
    if unresolved:
        raise SystemExit(
            f"V145 target is not safe to move. function={TARGET_FUNCTION} unresolved={unresolved}"
        )

    backup(legacy, "v144_before_v145")
    backup(stages, "v144_before_v145")
    ...
    data = {
        "schema_version": "aquanova.refactor.v145.ro_stages_stabilize_after_flow_commit_extract",
        ...
        "legacy": str(legacy),
        "stages": str(stages),
        "note": "Strict V139 has no ready candidates. V145 moves a small no-unknown-global connector and bridges its larger legacy dependencies.",
    }
    manifest.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")

    print("V145 RO stages stabilize-after-flow extraction applied")
    print("moved_count=1")

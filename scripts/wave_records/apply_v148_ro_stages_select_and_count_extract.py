...
TARGET_FUNCTIONS = ["_select_pass", "_set_stage_count"]
TARGET_FILE_REL = Path("ro") / "stages.py"

IMPORT_MARKER_START = "# V148_RO_STAGES_IMPORT_START"
IMPORT_MARKER_END = "# V148_RO_STAGES_IMPORT_END"
EXTRACT_MARKER = "# V148_RO_STAGES_SELECT_AND_COUNT_APPLIED"
MANIFEST_NAME = "v148_ro_stages_select_and_count_manifest.json"
...
def wave_automation_error_is_raise_factory_safe(nodes: list[ast.AST]) -> bool:
    # Accept only raise WaveAutomationError(...). If it is used in except,
    # isinstance, class bases, etc., a lazy factory would be unsafe.
    for node in nodes:
...
def main() -> int:
    ...
    if missing:
        raise SystemExit(f"V148 target functions not found in wave_ro_engine_legacy.py: {missing}")

    target_nodes = [funcs[name] for name in TARGET_FUNCTIONS]
    ...
    for node in target_nodes:
        all_free |= free_names(node)

    needs_wave_error_factory = EXCEPTION_FACTORY_NAME in all_free
    if needs_wave_error_factory and not wave_automation_error_is_raise_factory_safe(target_nodes):
        raise SystemExit("V148 target uses WaveAutomationError in a non-raise-call context; lazy factory would be unsafe.")

    explicit_import_deps = sorted(
        name for name in all_free
        if name in imports and name not in stages_defs and name not in TARGET_FUNCTIONS and name != EXCEPTION_FACTORY_NAME
    )
    bridged_legacy_refs = sorted(
        name for name in all_free
        if name in funcs and name not in TARGET_FUNCTIONS and name not in stages_defs and name != EXCEPTION_FACTORY_NAME
    )
    ...
    if unresolved:
        raise SystemExit(
            f"V148 targets are not safe to move. functions={TARGET_FUNCTIONS} unresolved={unresolved}"
        )

    backup(legacy, "v147_before_v148")
    backup(stages, "v147_before_v148")

    legacy_lines = legacy_text.splitlines()
    sources = [get_segment(legacy_lines, funcs[name]).rstrip() for name in TARGET_FUNCTIONS]
    ...
    data = {
        "schema_version": "aquanova.refactor.v148.ro_stages_select_and_count_extract",
        ...
        "legacy": str(legacy),
        "stages": str(stages),
        "note": "Moves small pass/stage-count helpers and uses a lazy WaveAutomationError factory only for raise WaveAutomationError(...) contexts.",
    }
    manifest.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")

    print("V148 RO stages select/count extraction applied")
    print(f"moved_count={len(TARGET_FUNCTIONS)}")

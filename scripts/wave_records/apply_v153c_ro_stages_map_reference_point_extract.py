...
LEGACY_ATTR_BRIDGES = {"_get_window_rect"}
...
def make_legacy_attr_bridge(name: str) -> str:
    # Does not require the helper to be statically visible as a top-level legacy function.
    # This covers helpers imported into legacy or attached by broader compatibility imports.
    return (
...
def main() -> int:
    ...
    if EXTRACT_MARKER in stages_text and manifest.exists():
        print("V153/V153C already applied")
    ...
    bridged_legacy_refs = sorted(
        ...
        if name in LEGACY_ATTR_BRIDGES and name not in stages_defs
    )
    ...
    if unresolved:
        raise SystemExit(
            f"V153C target is not safe to move. function={TARGET_FUNCTION} unresolved={unresolved}"
        )

    backup(legacy, "v152_before_v153c")
    backup(stages, "v152_before_v153c")
    ...
    for ref in bridged_legacy_refs:
        if ref not in stages_defs_after_bridge_remove:
    ...
    data = {
        "schema_version": "aquanova.refactor.v153c.ro_stages_map_reference_point_extract",
        ...
        "legacy": str(legacy),
        "stages": str(stages),
        "note": "Moves _map_reference_point into ro.stages. V153C late-binds _get_window_rect as a legacy attribute and handles reference constants by assignment/import/lazy getter.",
    }
    manifest.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")

    print("V153C RO stages map-reference-point extraction applied")
    print("moved_count=1")

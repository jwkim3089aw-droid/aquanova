...
raw = ScenarioResult()
disabled, disabled_report = apply_wave_runtime_corrections_to_output(raw, residual_layer, options={}, config={"enabled": False})
assert disabled is raw, disabled_report
assert disabled_report["status"] == "disabled", disabled_report

corrected, report = apply_wave_runtime_corrections_to_output(
    raw, residual_layer, options={"enable_wave_correction": True}, config={"enabled": False}
)
assert report["applied_count"] == 1, report
assert abs(corrected.passes[0].feed_pressure_bar - 9.0) < 1e-9, corrected.passes[0].feed_pressure_bar
assert raw.passes[0].feed_pressure_bar == 10.0, "raw object must not be mutated"

print("V118B runtime compatibility selftest PASS")

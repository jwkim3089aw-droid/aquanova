...
TARGET_FUNCTION = "_verify_stage_grid_membranes"
TARGET_FILE_REL = Path("ro") / "membrane.py"

FALLBACK_IMPORT_MARKER_START = "# V160_RO_MEMBRANE_IMPORT_START"
FALLBACK_IMPORT_MARKER_END = "# V160_RO_MEMBRANE_IMPORT_END"
EXTRACT_MARKER = "# V160_RO_MEMBRANE_VERIFY_STAGE_GRID_MEMBRANES_APPLIED"
MANIFEST_NAME = "v160_ro_membrane_verify_stage_grid_membranes_manifest.json"
...
def import_marker_pairs() -> list[tuple[str, str]]:
    return [
        ("# V160_RO_MEMBRANE_IMPORT_START", "# V160_RO_MEMBRANE_IMPORT_END"),
...
def main() -> int:
    ...
    if "WaveAutomationError" in except_handler_names(target_node):
        raise SystemExit("V160 refused: WaveAutomationError is used as an except-handler type in target.")

    safe_direct_import_deps = sorted(
        name for name in target_free
        if name in SAFE_DIRECT_IMPORTS and name not in membrane_defs
    )
    bridged_ro_refs = sorted(
        name for name in target_free
        if name in ro_index
        and name not in membrane_defs
    ...
    if unresolved:
        raise SystemExit(
            f"V160 target is not safe to move. function={TARGET_FUNCTION} unresolved={unresolved}"
        )

    backup(legacy, "v159_before_v160")
    backup(membrane, "v159_before_v160")
    ...
    data = {
        "schema_version": "aquanova.refactor.v160.ro_membrane_verify_stage_grid_membranes_extract",
        ...
        "note": "Moves _verify_stage_grid_membranes into ro.membrane.",
    }
    manifest.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")

    print("V160 RO membrane verify-stage-grid-membranes extraction applied")
    print("moved_count=1")

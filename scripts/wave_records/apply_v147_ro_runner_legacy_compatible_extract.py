...
TARGET_FUNCTION = "_legacy_compatible"
TARGET_FILE_REL = Path("ro") / "runner.py"

IMPORT_MARKER_START = "# V147_RO_RUNNER_IMPORT_START"
IMPORT_MARKER_END = "# V147_RO_RUNNER_IMPORT_END"
EXTRACT_MARKER = "# V147_RO_RUNNER_LEGACY_COMPATIBLE_APPLIED"
MANIFEST_NAME = "v147_ro_runner_legacy_compatible_manifest.json"

# Already-extracted helper in ro.feedwater. Use runtime bridge to avoid coupling
# runner import order to feedwater import order.
BRIDGEABLE_RO_FEEDWATER_REFS = {
...
def ensure_runner_header(text: str) -> str:
    if text.strip():
        ...
        return text.rstrip() + "\n\n"
    return (
        '"""RO runner compatibility helpers extracted from wave_ro_engine_legacy."""\n'
        "from __future__ import annotations\n\n"
    )


def make_feedwater_bridge(name: str) -> str:
    return (
        "def " + name + "(*args, **kwargs):\n"
        "    \"\"\"Bridge to ro.feedwater while keeping runner import-order independent.\"\"\"\n"
        "    try:\n"
        "        from ro.feedwater import " + name + " as _impl\n"
        "    except ImportError:\n"
        "        from .feedwater import " + name + " as _impl\n"
...
def main() -> int:
    ...
    bridged_feedwater_refs = sorted(name for name in target_free if name in BRIDGEABLE_RO_FEEDWATER_REFS)

    unresolved = (
        target_free
        - set(bridged_feedwater_refs)
        - runner_defs
        - ALLOWED_GLOBALS
    )
    ...
    if unresolved:
        raise SystemExit(
            f"V147 target is not safe to move. function={TARGET_FUNCTION} unresolved={unresolved}"
        )

    backup(legacy, "v146_before_v147")
    backup(runner, "v146_before_v147")
    ...
    existing_defs = module_defined_names(ast.parse(runner_text, filename=str(runner)))
    bridge_blocks = []
    for ref in bridged_feedwater_refs:
        if ref not in existing_defs:
            bridge_blocks.append(make_feedwater_bridge(ref))
    ...
    data = {
        "schema_version": "aquanova.refactor.v147.ro_runner_legacy_compatible_extract",
        "generated_at": datetime.now().isoformat(timespec="seconds"),
        "moved_count": 1,
        "moved_functions": [TARGET_FUNCTION],
        "bridged_feedwater_refs": bridged_feedwater_refs,
        "legacy": str(legacy),
        "runner": str(runner),
        "note": "Moves the small bridge-aware _legacy_compatible helper into ro.runner.",
    }
    manifest.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")

    print("V147 RO runner legacy-compatible extraction applied")
    print("moved_count=1")
    print(f"moved_functions={TARGET_FUNCTION}")
    print("bridged_feedwater_refs=" + (", ".join(bridged_feedwater_refs) if bridged_feedwater_refs else ""))
    print(f"manifest={manifest}")
    return 0

...
TARGET_FUNCTION = "enter_summary_report_case"
TARGET_FILE_REL = Path("ro") / "reports.py"

FALLBACK_IMPORT_MARKER_START = "# V156_RO_REPORTS_IMPORT_START"
FALLBACK_IMPORT_MARKER_END = "# V156_RO_REPORTS_IMPORT_END"
EXTRACT_MARKER = "# V156_RO_REPORTS_ENTER_SUMMARY_REPORT_CASE_APPLIED"
MANIFEST_NAME = "v156_ro_reports_enter_summary_report_case_manifest.json"

LEGACY_ATTR_BRIDGES = {"_repair_missing_element_type_dialog"}
...
def import_marker_pairs() -> list[tuple[str, str]]:
    return [
        ("# V156_RO_REPORTS_IMPORT_START", "# V156_RO_REPORTS_IMPORT_END"),
        ("# V135_RO_REPORTS_IMPORT_START", "# V135_RO_REPORTS_IMPORT_END"),
    ]
...
def ensure_reports_header(text: str) -> str:
    if text.strip():
        if "from __future__ import annotations" not in text:
            return 'from __future__ import annotations\n\n' + text.rstrip() + "\n\n"
        return text.rstrip() + "\n\n"
    return (
        '"""RO report-entry helpers extracted from wave_ro_engine_legacy."""\n'
        "from __future__ import annotations\n\n"
    )
...
def main() -> int:
    ...
    reports = wr / TARGET_FILE_REL
    manifest = wr / "ro" / MANIFEST_NAME

    if not legacy.exists():
        raise SystemExit("wave_ro_engine_legacy.py not found. Apply V135 first.")
    if not reports.exists():
        raise SystemExit("ro/reports.py not found. Apply V135 first.")

    legacy_text = read_text(legacy)
    reports_text = read_text(reports)

    if EXTRACT_MARKER in reports_text and manifest.exists():
        print("V156 already applied")
    ...
    reports_tree = ast.parse(reports_text, filename=str(reports))
    reports_defs = module_defined_names(reports_tree)

    target_free = free_names(target_node)

    if "WaveAutomationError" in except_handler_names(target_node):
        raise SystemExit("V156 refused: WaveAutomationError is used as an except-handler type in target.")

    bridged_ro_refs = sorted(
        name for name in target_free
        if name in ro_index and name not in reports_defs and name != TARGET_FUNCTION
    )
    explicit_import_deps = sorted(
        name for name in target_free
        if name in imports and name not in reports_defs and name not in bridged_ro_refs
    )
    bridged_legacy_refs = sorted(
        name for name in target_free
        if name in LEGACY_ATTR_BRIDGES and name not in reports_defs
    )
    ...
    if unresolved:
        raise SystemExit(
            f"V156 target is not safe to move. function={TARGET_FUNCTION} unresolved={unresolved}"
        )

    backup(legacy, "v155_before_v156")
    backup(reports, "v155_before_v156")

    source = get_segment(legacy_lines, target_node).rstrip()

    reports_text = ensure_reports_header(reports_text)
    reports_defs_after = module_defined_names(ast.parse(reports_text, filename=str(reports)))

    blocks = []
    for ref in bridged_ro_refs:
        if ref not in reports_defs_after:
            module = ro_index[ref]
            if module == "reports":
                continue
    ...
    for dep in explicit_import_deps:
        ...
        if block and block not in reports_text and block not in seen_import_blocks:
            blocks.append(block)
    ...
    py_compile.compile(str(reports), doraise=True)
    py_compile.compile(str(legacy), doraise=True)

    active_start, _ = active_import_markers(read_text(legacy))
    data = {
        "schema_version": "aquanova.refactor.v156.ro_reports_enter_summary_report_case_extract",
        ...
        "legacy": str(legacy),
        "reports": str(reports),
        "note": "Moves enter_summary_report_case into ro.reports using runtime bridges and copied UI imports.",
    }
    manifest.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")

    print("V156 RO reports enter-summary-report-case extraction applied")
    print("moved_count=1")

...
TARGET_FUNCTION = "_reconcile_ro_pass_topology"
TARGET_FILE_REL = Path("ro") / "membrane.py"

FALLBACK_IMPORT_MARKER_START = "# V157_RO_MEMBRANE_IMPORT_START"
FALLBACK_IMPORT_MARKER_END = "# V157_RO_MEMBRANE_IMPORT_END"
EXTRACT_MARKER = "# V157_RO_MEMBRANE_RECONCILE_PASS_TOPOLOGY_APPLIED"
MANIFEST_NAME = "v157_ro_membrane_reconcile_pass_topology_manifest.json"

# These are safe module-level imports if the legacy import source is not statically captured.
SAFE_DIRECT_IMPORTS = {
...
def import_marker_pairs() -> list[tuple[str, str]]:
    return [
        ("# V157_RO_MEMBRANE_IMPORT_START", "# V157_RO_MEMBRANE_IMPORT_END"),
...
def ensure_membrane_header(text: str) -> str:
    if text.strip():
        ...
        return text.rstrip() + "\n\n"
    return (
        '"""RO membrane/topology helpers extracted from wave_ro_engine_legacy."""\n'
        "from __future__ import annotations\n\n"
    )
...
def main() -> int:
    ...
    # A function-style WaveAutomationError bridge is safe for raises, but not for
    # `except WaveAutomationError:`. Refuse if an exception handler uses it.
    if "WaveAutomationError" in except_handler_names(target_node):
        raise SystemExit("V157 refused: WaveAutomationError is used as an except-handler type in target.")

    bridged_ro_refs = sorted(
        name for name in target_free
        if name in ro_index and name not in membrane_defs and name != TARGET_FUNCTION
    )
    explicit_import_deps = sorted(
        name for name in target_free
        if name in imports and name not in membrane_defs and name not in bridged_ro_refs
    )
    safe_direct_import_deps = sorted(
        name for name in target_free
        if name in SAFE_DIRECT_IMPORTS and name not in membrane_defs and name not in explicit_import_deps
    )
    ...
    if unresolved:
        raise SystemExit(
            f"V157 target is not safe to move. function={TARGET_FUNCTION} unresolved={unresolved}"
        )

    backup(legacy, "v156_before_v157")
    backup(membrane, "v156_before_v157")

    source = get_segment(legacy_lines, target_node).rstrip()

    membrane_text = ensure_membrane_header(membrane_text)
    membrane_defs_after = module_defined_names(ast.parse(membrane_text, filename=str(membrane)))
    ...
    for dep in explicit_import_deps:
        ...
        if block and block not in membrane_text and block not in seen_import_blocks:
            blocks.append(block)
            seen_import_blocks.add(block)

    for dep in safe_direct_import_deps:
        block = SAFE_DIRECT_IMPORTS[dep]
        if block not in membrane_text and block not in blocks:
            blocks.append(block)

    for ref in bridged_ro_refs:
        if ref not in membrane_defs_after:
            module = ro_index[ref]
            if module == "membrane":
                continue
    ...
    data = {
        "schema_version": "aquanova.refactor.v157.ro_membrane_reconcile_pass_topology_extract",
        ...
        "note": "Moves _reconcile_ro_pass_topology into ro.membrane and handles logging as an explicit safe import.",
    }
    manifest.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")

    print("V157 RO membrane reconcile-pass-topology extraction applied")
    print("moved_count=1")

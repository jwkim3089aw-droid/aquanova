...
manifest = wr / "batch" / "v134b_plan_schema_cluster_extraction_manifest.json"
...
assert "# V134B_PLAN_SCHEMA_CLUSTER_APPLIED" in pt
assert "import copy" in pt

for name in moved:
    if name == "WaveAutomationError":
        assert "class WaveAutomationError" not in lt, "WaveAutomationError still in legacy"
        assert "class WaveAutomationError" in pt, "WaveAutomationError class missing from plan_schema"
    elif name in {"_canonical_temperature_mode", "_temperature_variant_suffix", "_clone_case_for_global_temperature", "expand_cases_for_wave_global_temperature"}:
        assert f"def {name}(" not in lt, f"{name} still in legacy"
        assert f"def {name}(" in pt, f"{name} missing from plan_schema"

# STATE-dependent function should remain in legacy.
assert "def _write_two_case_summary(" in lt
...
print("V134B wave_batch plan_schema import-aware extraction selftest PASS")

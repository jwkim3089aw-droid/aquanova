...
TARGET_FUNCTIONS = ['_canonical_temperature_mode', '_temperature_variant_suffix', '_choose_stable_global_temperature_mode', '_clone_case_for_global_temperature', '_settings_from_case', '_write_two_case_summary', 'expand_cases_for_wave_global_temperature', 'run_two_ro_cases']
PLAN_SCHEMA_HEADER = '"""Batch plan/case schema helpers extracted by V131A.\n\nThe legacy module imports these names back for compatibility.\n"""\nfrom __future__ import annotations\n\nimport csv\nimport json\nimport math\nimport os\nimport re\nimport time\nfrom collections import Counter, defaultdict\nfrom datetime import datetime\nfrom pathlib import Path\nfrom typing import Any, Dict, Iterable, Iterator, List, Mapping, MutableMapping, Optional, Sequence, Set, Tuple, Union\n\ntry:\n    from wave_ro_schema import ROCaseConfig\nexcept ImportError:\n    try:\n        from ..wave_ro_schema import ROCaseConfig\n    except ImportError:\n        ROCaseConfig = None  # type: ignore\n\n'
...
def main() -> int:
    ...
    if IMPORT_MARKER_START in legacy_text and manifest_path.exists():
        print("V131A already appears applied")
    ...
    if not moved_names:
        raise SystemExit(f"No safe plan_schema functions could be moved. Skipped={skipped}")

    moved_nodes = [funcs[name] for name in moved_names]
    moved_sources = [get_segment(lines, funcs[name]) for name in moved_names]

    backup(legacy, "v131_failed_before_v131a")
    backup(plan_schema, "v131_failed_before_v131a")

    plan_schema_text = PLAN_SCHEMA_HEADER + "\n\n".join(src.rstrip() for src in moved_sources) + "\n"
    plan_schema.write_text(plan_schema_text, encoding="utf-8")
    ...
    manifest = {
        "schema_version": "aquanova.refactor.v131a.plan_schema_extraction",
        ...
        "skipped": skipped,
        "note": "V131 failed before modifying code due an unquoted HEADER literal. V131A fixes it and uses safe dependency checks.",
    }
    manifest_path.write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")

    print("V131A wave_batch plan_schema extraction applied")

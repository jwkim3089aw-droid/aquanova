...
TARGET_FUNCTION = "_stage_grid_points"
TARGET_FILE_REL = Path("ro") / "stages.py"

IMPORT_MARKER_START = "# V140_RO_STAGES_IMPORT_START"
IMPORT_MARKER_END = "# V140_RO_STAGES_IMPORT_END"
EXTRACT_MARKER = "# V140_RO_STAGES_STAGE_GRID_POINTS_APPLIED"
MANIFEST_NAME = "v140_ro_stages_stage_grid_points_manifest.json"
...
def make_legacy_bridge(name: str) -> str:
    return (
        ...
        "    \"\"\"Bridge to the legacy helper left in wave_ro_engine_legacy during staged refactor.\"\"\"\n"
        "    try:\n"
...
def main() -> int:
    ...
    internal_refs = sorted(target_free & set(funcs))
    bridged_refs = [name for name in internal_refs if name != TARGET_FUNCTION]
    ...
    if unresolved:
        raise SystemExit(
            f"V140 target is not safe to move. function={TARGET_FUNCTION} unresolved={unresolved}"
        )

    backup(legacy, "v138_before_v140")
    backup(stages, "v138_before_v140")

    legacy_lines = legacy_text.splitlines()
    source = get_segment(legacy_lines, target_node).rstrip()

    bridge_blocks = []
    existing_stages_defs = module_defined_names(ast.parse(stages_text, filename=str(stages)))
    for ref in bridged_refs:
        if ref not in existing_stages_defs:
    ...
    data = {
        "schema_version": "aquanova.refactor.v140.ro_stages_stage_grid_points_extract",
        ...
        "legacy": str(legacy),
        "stages": str(stages),
        "note": "Extracts _stage_grid_points and bridges any one-hop same-file helpers left in legacy.",
    }
    manifest.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")

    print("V140 RO stages leaf extraction applied")
    print("moved_count=1")

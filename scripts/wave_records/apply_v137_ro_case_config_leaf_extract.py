...
TARGET_MODULE_IMPORT = "ro.case_config"
TARGET_FILE_REL = Path("ro") / "case_config.py"

IMPORT_MARKER_START = "# V137_RO_CASE_CONFIG_IMPORT_START"
IMPORT_MARKER_END = "# V137_RO_CASE_CONFIG_IMPORT_END"
EXTRACT_MARKER = "# V137_RO_CASE_CONFIG_LEAF_APPLIED"
MANIFEST_NAME = "v137_ro_case_config_leaf_manifest.json"
...
def ensure_case_config_header(text: str) -> str:
    if "V137_RO_CASE_CONFIG_LEAF_APPLIED" in text:
        return text
    if text.strip():
        return text.rstrip() + "\n\n"
    return (
...
def main() -> int:
    ...
    if unresolved:
        raise SystemExit(
            f"V137 target is not dependency-free enough to move safely. "
            f"function={TARGET_FUNCTION} unresolved={unresolved}"
        )

    backup(legacy, "v136_before_v137")
    backup(case_config, "v136_before_v137")
    ...
    data = {
        "schema_version": "aquanova.refactor.v137.ro_case_config_leaf_extract",
        "generated_at": datetime.now().isoformat(timespec="seconds"),
        "moved_count": 1,
        "moved_functions": [TARGET_FUNCTION],
        "legacy": str(legacy),
        "case_config": str(case_config),
        "note": "Extracts a single dependency-free RO case settings helper into ro.case_config.",
    }
    manifest.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")

    print("V137 RO case_config leaf extraction applied")
    print("moved_count=1")

...
TARGET_FUNCTION = "_restore_stage_topologies_after_flow_commit"
TARGET_FILE_REL = Path("ro") / "stages.py"

FALLBACK_IMPORT_MARKER_START = "# V158_RO_STAGES_IMPORT_START"
FALLBACK_IMPORT_MARKER_END = "# V158_RO_STAGES_IMPORT_END"
EXTRACT_MARKER = "# V158_RO_STAGES_RESTORE_TOPOLOGIES_APPLIED"
MANIFEST_NAME = "v158_ro_stages_restore_topologies_manifest.json"
...
def import_marker_pairs() -> list[tuple[str, str]]:
    return [
        ("# V158_RO_STAGES_IMPORT_START", "# V158_RO_STAGES_IMPORT_END"),
...
def main() -> int:
    ...
    if "WaveAutomationError" in except_handler_names(target_node):
        raise SystemExit("V158 refused: WaveAutomationError is used as an except-handler type in target.")

    bridged_ro_refs = sorted(
        name for name in target_free
        if name in ro_index and name not in stages_defs and name != TARGET_FUNCTION
    )
    explicit_import_deps = sorted(
        name for name in target_free
        if name in imports and name not in stages_defs and name not in bridged_ro_refs
    )
    safe_direct_import_deps = sorted(
        name for name in target_free
        if name in SAFE_DIRECT_IMPORTS and name not in stages_defs and name not in explicit_import_deps
    )
    ...
    if unresolved:
        raise SystemExit(
            f"V158 target is not safe to move. function={TARGET_FUNCTION} unresolved={unresolved}"
        )

    backup(legacy, "v157_before_v158")
    backup(stages, "v157_before_v158")

    source = get_segment(legacy_lines, target_node).rstrip()

    stages_text = ensure_stages_header(stages_text)
    # Replace the V145A bridge def with the real function if that bridge exists.
    stages_text = remove_existing_target_defs(stages_text, {TARGET_FUNCTION})
    stages_defs_after = module_defined_names(ast.parse(stages_text, filename=str(stages)))
    ...
    for dep in safe_direct_import_deps:
        ...
        if block not in stages_text and block not in blocks:
            blocks.append(block)

    for ref in bridged_ro_refs:
        if ref not in stages_defs_after:
            module = ro_index[ref]
            if module == "stages":
                continue
    ...
    data = {
        "schema_version": "aquanova.refactor.v158.ro_stages_restore_topologies_extract",
        ...
        "legacy": str(legacy),
        "stages": str(stages),
        "note": "Moves _restore_stage_topologies_after_flow_commit into ro.stages and replaces its earlier legacy bridge.",
    }
    manifest.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")

    print("V158 RO stages restore-topologies extraction applied")
    print("moved_count=1")

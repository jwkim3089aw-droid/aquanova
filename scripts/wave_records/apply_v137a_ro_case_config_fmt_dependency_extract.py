...
TARGET_FUNCTIONS = ["_fmt_value", "_settings_from_ro_case"]
TARGET_FILE_REL = Path("ro") / "case_config.py"

IMPORT_MARKER_START = "# V137A_RO_CASE_CONFIG_IMPORT_START"
IMPORT_MARKER_END = "# V137A_RO_CASE_CONFIG_IMPORT_END"
EXTRACT_MARKER = "# V137A_RO_CASE_CONFIG_FMT_DEP_APPLIED"
MANIFEST_NAME = "v137a_ro_case_config_fmt_dependency_manifest.json"
...
def parse_imported_names(text: str) -> list[str]:
    names: list[str] = []
    # Preserve prior V137 marker if a user somehow applied a partial/manual version.
...
def replace_or_insert_import(text: str, names: list[str]) -> str:
    # Replace old V137 block if present.
    for start_marker, end_marker in [
        ("# V137_RO_CASE_CONFIG_IMPORT_START", "# V137_RO_CASE_CONFIG_IMPORT_END"),
        (IMPORT_MARKER_START, IMPORT_MARKER_END),
    ]:
...
def main() -> int:
    ...
    if missing:
        raise SystemExit(f"V137A target functions not found in wave_ro_engine_legacy.py: {missing}")

    case_tree = ast.parse(case_text, filename=str(case_config))
    case_defs = module_defined_names(case_tree)
    moved_set = set(TARGET_FUNCTIONS)

    unresolved_by_function: dict[str, list[str]] = {}
    for name in TARGET_FUNCTIONS:
        unresolved = (
            free_names(funcs[name])
            - moved_set
            - case_defs
            - ALLOWED_GLOBALS
        )
        ...
        if unresolved:
            unresolved_by_function[name] = unresolved

    if unresolved_by_function:
        raise SystemExit(
            "V137A targets are not safe to move. "
            + json.dumps(unresolved_by_function, ensure_ascii=False)
        )

    backup(legacy, "v137_failed_before_v137a")
    backup(case_config, "v137_failed_before_v137a")
    ...
    data = {
        "schema_version": "aquanova.refactor.v137a.ro_case_config_fmt_dependency_extract",
        "generated_at": datetime.now().isoformat(timespec="seconds"),
        "moved_count": len(TARGET_FUNCTIONS),
        "moved_functions": TARGET_FUNCTIONS,
        "legacy": str(legacy),
        "case_config": str(case_config),
        "note": "V137 failed safely because _settings_from_ro_case depends on _fmt_value. V137A moves both as a small leaf pair.",
    }
    manifest.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")

    print("V137A RO case_config fmt dependency extraction applied")

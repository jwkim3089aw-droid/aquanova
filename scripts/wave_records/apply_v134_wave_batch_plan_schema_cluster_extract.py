...
TARGET_FUNCTIONS = ['_canonical_temperature_mode', '_temperature_variant_suffix', '_clone_case_for_global_temperature', 'expand_cases_for_wave_global_temperature', '_write_two_case_summary']

IMPORT_MARKER_START = "# V131A_PLAN_SCHEMA_IMPORT_START"
IMPORT_MARKER_END = "# V131A_PLAN_SCHEMA_IMPORT_END"
MANIFEST_NAME = "v134_plan_schema_cluster_extraction_manifest.json"
...
def resolve_safe_move_set(
    legacy_funcs: dict[str, ast.AST],
    plan_schema_existing_defs: set[str],
) -> tuple[list[str], list[dict]]:
    candidates = [name for name in TARGET_FUNCTIONS if name in legacy_funcs]
...
def main() -> int:
    ...
    if "V134_PLAN_SCHEMA_CLUSTER_APPLIED" in legacy_text and manifest.exists():
    ...
    legacy_funcs = top_level_functions(legacy_tree)
    plan_existing_defs = module_defined_names(plan_tree)

    moved_names, skipped = resolve_safe_move_set(legacy_funcs, plan_existing_defs)
    if not moved_names:
        raise SystemExit(f"No safe V134 plan_schema cluster functions could be moved. Skipped={skipped}")

    backup(legacy, "v132_before_v134")
    backup(plan_schema, "v132_before_v134")

    legacy_lines = legacy_text.splitlines()
    moved_sources = [get_segment(legacy_lines, legacy_funcs[name]).rstrip() for name in moved_names]
    if not plan_text.endswith("\n"):
        plan_text += "\n"
    plan_text = plan_text.rstrip() + "\n\n# V134_PLAN_SCHEMA_CLUSTER_APPLIED\n\n" + "\n\n".join(moved_sources) + "\n"
    plan_schema.write_text(plan_text, encoding="utf-8")

    reduced = remove_spans(legacy_lines, [legacy_funcs[name] for name in moved_names])
    ...
    data = {
        "schema_version": "aquanova.refactor.v134.plan_schema_cluster_extraction",
        ...
        "note": "Extracts remaining safe temperature/case expansion cluster into batch.plan_schema.",
    }
    manifest.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")

    print("V134 wave_batch plan_schema cluster extraction applied")

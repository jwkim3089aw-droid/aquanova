...
IMPORT_MARKER_START = "# V143_RO_CASE_CONFIG_IMPORT_START"
IMPORT_MARKER_END = "# V143_RO_CASE_CONFIG_IMPORT_END"
EXTRACT_MARKER = "# V143_RO_CASE_CONFIG_VERIFY_INPUTS_APPLIED"
MANIFEST_NAME = "v143_ro_case_config_verify_inputs_manifest.json"
...
def main() -> int:
    ...
    explicit_import_deps = sorted(
        ...
        if name in imports and name not in case_defs
    )
    ...
    if unresolved:
        raise SystemExit(
            f"V143 target is not safe to move. function={TARGET_FUNCTION} unresolved={unresolved}"
        )

    backup(legacy, "v142_before_v143")
    backup(case_config, "v142_before_v143")
    ...
    data = {
        "schema_version": "aquanova.refactor.v143.ro_case_config_verify_inputs_extract",
        ...
        "note": "Extracts _verify_case_operating_inputs after _capture_case_ro_state moved into ro.case_config.",
    }
    manifest.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")

    print("V143 RO case_config operating-input verification extraction applied")
    print("moved_count=1")
